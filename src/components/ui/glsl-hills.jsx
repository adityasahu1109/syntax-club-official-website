...
const GLSLHills = ({ width = '100vw', height = '100vh', cameraZ = 125, planeSize = 256, speed = 0.5 }) => {
  ...
  useEffect(() => {
    ...
    const resize = () => {
      ...
      // Responsive adjustments for mobile phone view
      if (window.innerWidth < 768) {
        camera.position.set(0, 18, cameraZ * 1.05);
        camera.lookAt(new THREE.Vector3(0, 5, 0)); // Lower look target shifts the animation up on screen
      } else {
  ...
  return (
    <div ref={containerRef} className="absolute inset-0 pointer-events-none" style={{ position: 'absolute', width: '100%', height: '100%' }}> 
      <canvas
        ref={canvasRef}
