...
const ProjectArticle = () => {
  ...
  useEffect(() => {
    async function fetchProject() {
      ...
      if (fetchError && slug.length === 36) {
        ...
        const fallbackQuery = await supabase.from('projects').select('*').eq('id', slug).single();
  ...
  if (error || !project) {
    return (
      <div className="w-full min-h-[60vh] flex flex-col items-center justify-center p-6 gap-4">
        ...
        <h2 className="text-2xl font-black text-white">{error || "Project Analysis not found"}</h2>
        <Link to="/projects" className="text-primary hover:underline uppercase text-xs font-bold tracking-widest mt-4 flex items-center gap-2">
  ...
  return (
    <div className="max-w-7xl mx-auto px-6 lg:px-12 py-12 w-full">
      <Link to="/projects" className="text-primary hover:text-white transition-colors uppercase text-xs font-bold tracking-widest mb-8 inline-flex items-center gap-2">
        <span className="material-symbols-outlined text-sm">arrow_back</span>
        Back to Projects
      </Link>
      ...
      <article className="bg-white/5 border border-white/10 rounded-2xl overflow-hidden backdrop-blur-sm shadow-[0_0_30px_rgba(6,245,249,0.05)]">
        {/* Header Image */}
        {project.imageUrl && (
          <div className="w-full aspect-[21/9] md:aspect-[3/1] relative border-b border-white/10 bg-black/40">
        ...
        {/* Content Box */}
        <div className="p-8 md:p-12 relative -mt-20 md:-mt-32 z-10 flex flex-col md:flex-row gap-12">
          ...
          <div className="flex-1 min-w-0">
            ...
            <div className="markdown-body">
              {project.content ? (
                ...
                <p className="text-slate-500 italic p-6 border border-dashed border-white/20 rounded-xl bg-white/5 text-center">
                  Technical analysis and challenge documentation for this project has not been written yet.
                </p>
              )}
            </div>
          </div>
          
          {/* Right Sidebar logic for External Links */}
          <div className="w-full md:w-64 flex flex-col gap-4 md:mt-24">
             {project.githubUrl && (
                <a
                  href={project.githubUrl}
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 py-4 rounded-xl border border-white/20 text-white font-bold text-xs uppercase tracking-wider hover:border-primary hover:text-primary transition-all w-full"
                >
              ...
              {project.liveUrl && (
                <a
                  ...
                  target="_blank"
                  rel="noopener noreferrer"
                  className="flex items-center justify-center gap-2 py-4 rounded-xl bg-primary text-black font-black shadow-[0_0_15px_rgba(6,245,249,0.3)] text-xs uppercase tracking-widest hover:bg-white hover:shadow-[0_0_20px_rgba(255,255,255,0.5)] transition-all w-full"
                >
